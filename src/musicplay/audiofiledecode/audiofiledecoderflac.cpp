...
namespace eink {
    ...
    AudiofileDecoderFlac *AudiofileDecoderFlac::CurMy = NULL;

    AudiofileDecoderFlac::AudiofileDecoderFlac(QObject *parent)
    ...
    AudiofileDecoderFlac::~AudiofileDecoderFlac()
	{
        qDebug() << "AudiofileDecoderFlac::~AudiofileDecoderFlac()audioq = " << packet_queue_Size(&d->audioq);
    ...
    void AudiofileDecoderFlac::startParse()
    {
        qDebug() << "AudiofileDecoderFlac::startParse() @@@@@@@@@@@@";
    ...
    void AudiofileDecoderFlac::InitEvn()
    {
        d->audioIndex = -1;
	    d->subtitleIndex = -1;
	    audio_buf = NULL;
    ...
    void AudiofileDecoderFlac::findVedionStreamType()
    {
	    ...
	    if( ret < 0 ) {
            printf("AudiofileDecoderFlac::findVedionStreamType() error info!!\n");

	        avformat_close_input(&d->pFormatCtx);
	        return;
	    }

        qDebug() << "AudiofileDecoderFlac::findVedionStreamType() vedio lenth=" << d->pFormatCtx->duration
	    ...
	    for (int i = 0; i < d->pFormatCtx->nb_streams; i++) {
		    if(d->pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
			    d->audioIndex = i;
                printf("AudiofileDecoderFlac Find audio stream.\n");
		    } else if(d->pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE){
			    d->subtitleIndex = i;
                printf("AudiofileDecoderFlac Find subtitle stream.");
		    }
	    }

	    if (d->audioIndex == -1) {
		    printf("AudiofileDecoderFlac:: Didn't find a audio stream!!\n");
		    return;
	    }
        ...
        //qDebug() << "AudiofileDecoderFlac::findVedionStreamType Audio frame rate==" << frame_rate;

        d->time_base = stream->time_base;
        qDebug() << "AudiofileDecoderFlac::findVedionStreamType time_base num=" << d->time_base.num
                << d->time_base.den;

        createDecompiler();
    }

    void AudiofileDecoderFlac::createDecompiler()
	{
	    //查找解码器
        ...
        if( ret < 0) {
            printf("AudiofileDecoderFlac:: find avcodec faile!!!\n");
            avformat_close_input(&d->pFormatCtx);
            return;
        }
	    ...
	    if (d->pCodec == NULL){
		    printf("AudiofileDecoderFlac:: Codec not found\n");
		    avformat_close_input(&d->pFormatCtx);
		    return;
	    }
	    ...
	    if (avcodec_open2(d->pCodecCtx, d->pCodec, NULL) < 0) {
		    printf("AudiofileDecoderFlac:: Could not open codec\n");
    ...
    void AudiofileDecoderFlac::openAudioDevice()
	{
        qDebug() << "AudiofileDecoderFlac::readData() channels=" << d->pCodecCtx->channels
        ...
        qDebug() << "AudiofileDecoderFlac::OpenAudio() d->wanted_spec.channels== " <<
        ...
        qDebug() << "AudiofileDecoderFlac::OpenAudio() d->spec.channels== " <<
    ...
    void AudiofileDecoderFlac::readData()
    {
        //分配一个packet
        ...
        while(!d->isAudioPacketReadFinished)
        {
            ...
            if (packet->stream_index == d->audioIndex)
            {
                ...
                //如果没有这个就放不出声音,0是不暂停，非零是暂停
                    ...
                    qDebug() << "AudiofileDecoderFlac::readData() satrt play audio!!";
                }
            } else {
                av_free_packet(packet);
            }
        }
        qDebug() << "AudiofileDecoderFlac::readData packet finished!!!size="
...
//三个变量设置为static就是为了保存上次数据，也可以用全局变量，但是感觉这样更好。
    void AudiofileDecoderFlac::audio_callback(void *userdata, Uint8 *stream, int len)
    {
        //qDebug() << "AudiofileDecoderFlac::audio_callback play audio buffer size = " << len;
    ...
    int AudiofileDecoderFlac::audio_decode_frame(AVCodecContext *aCodecCtx)
    {
        static AVPacket pkt;
        ...
        if( packet_queue_empty(&d->audioq) && d->isAudioPacketReadFinished ) {
            qDebug() << "AudiofileDecoderFlac::audio_decode_frame finished!!!!";
            SDL_PauseAudio(1);
        ...
        for(;;)
        {
            while(audio_pkt_size > 0) {
                ...
                if(data_size <= 0) {
                    ...
                    qDebug() << "AudiofileDecoderFlac::audio_decode_frame data_size <= 0";
                    return -1;
                }
            ...
            if(packet_queue_get(&d->audioq, &pkt) < 0) {
                qDebug() << "AudiofileDecoderFlac::audio_decode_frame packet_queue_get(&d->audioq, &pkt) < 0";
                return -1;
            }
    ...
    void AudiofileDecoderFlac::setAudioDecoderState(bool flag)
    {
        d->isRuning = flag;
        qDebug() << "AudiofileDecoderFlac::setAudioDecoderState flag=" << flag;
    }

    bool AudiofileDecoderFlac::getAudioDecoderState()
    {
        return d->isRuning;
    }

    void AudiofileDecoderFlac::freeMemory()
    {
    ...
    int AudiofileDecoderFlac::getAudioPacketSize()
    {
        return packet_queue_Size(&d->audioq);
    }

    void AudiofileDecoderFlac::play()
    {
        d->isRuning = true;
        startParse();
    }

	void AudiofileDecoderFlac::stop()
	{
	    qDebug() << "AudiofileDecoderFlac::stop()!!!!!!!!!";
        d->isRuning = false;
        isMusicPlayFinished_GL = true;
        quitPlay();
        freeMemory();
	...
	void AudiofileDecoderFlac::suspendPlay()
	{
	    d->isRuning = false;
	    SDL_PauseAudio(1);
	}

	void AudiofileDecoderFlac::keepOnPlay()
	{
	    d->isRuning = true;
	    SDL_PauseAudio(0);
	}

	const char *AudiofileDecoderFlac::getAudioPlayerType()
	{
	    return AUDIO_MSG[FILE_FLAC].c_str();
	}

	void AudiofileDecoderFlac::setFilePath(const std::string &path)
	{
	    d->filePath = path;
	}

	bool AudiofileDecoderFlac::getPlayFinishedState()
	{
	    return d->isPLayFinished;
	}

	void AudiofileDecoderFlac::quitPlay()
	{
	...
	void AudiofileDecoderFlac::adjustPlayTimePos(int timePos)
	{
	    d->isRuning = false;
	    if(SDL_GetAudioStatus() == SDL_AUDIO_PLAYING) {
	        SDL_PauseAudio(1);
	...
	void AudiofileDecoderFlac::avSeekPacket(int timestamp)
    {
        ...
        if(ret < 0) {
            qDebug() << "AudiofileDecoderFlac::avSeekPacket failed!!!!!!!!!!";
            d->isRuning = true;
            SDL_PauseAudio(0);
            return;
        }
        ...
        while(!d->isAudioPacketReadFinished)
        {
            ...
            if (packet->stream_index == d->audioIndex)
            {
                ...
                //如果没有这个就放不出声音,0是不暂停，非零是暂停
                    ...
                    qDebug() << "AudiofileDecoderFlac::avSeekPacket satrt play audio!!";
                }
            } else {
                av_free_packet(packet);
            }
        }
        qDebug() << "AudiofileDecoderFlac::avSeekPacket finished ,,size="
                    << packet_queue_Size(&d->audioq);
	}


    void AudiofileDecoderFlac::createAudioFile()
    {
