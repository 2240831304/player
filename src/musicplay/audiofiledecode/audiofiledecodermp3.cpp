...
namespace eink {
    ...
    AudioFileDecoderMp3 *AudioFileDecoderMp3::CurMy = NULL;

    AudioFileDecoderMp3::AudioFileDecoderMp3(QObject *parent)
    ...
    AudioFileDecoderMp3::~AudioFileDecoderMp3()
	{
        qDebug() << "AudioFileDecoderMp3::~AudioFileDecoderMp3() audioq = " << packet_queue_Size(&d->audioq);
    ...
    void AudioFileDecoderMp3::startParse()
    {
        qDebug() << "AudioFileDecoderMp3::startParse() @@@@@@@@@@@@";
    ...
    void AudioFileDecoderMp3::InitEvn()
    {
        d->audioIndex = -1;
	    d->subtitleIndex = -1;
	    audio_buf = NULL;
    ...
    void AudioFileDecoderMp3::findVedionStreamType()
    {
	    ...
	    if( ret < 0 ) {
            printf("AudioFileDecoderMp3::findVedionStreamType() error info!!\n");

	        avformat_close_input(&d->pFormatCtx);
	        return;
	    }

        qDebug() << "AudioFileDecoderMp3::findVedionStreamType() vedio lenth=" << d->pFormatCtx->duration
	    ...
	    for (int i = 0; i < d->pFormatCtx->nb_streams; i++) {
		    if(d->pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
			    d->audioIndex = i;
                printf("AudioFileDecoderMp3 Find audio stream.\n");
		    } else if(d->pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE){
			    d->subtitleIndex = i;
                printf("AudioFileDecoderMp3 Find subtitle stream.");
		    }
	    }
        ...
        //qDebug() << "AudioFileDecoderMp3::findVedionStreamType Audio frame rate==" << frame_rate;

        d->time_base = stream->time_base;
        qDebug() << "AudioFileDecoderMp3::findVedionStreamType time_base num=" << d->time_base.num
                << d->time_base.den;

        createDecompiler();
    }

    void AudioFileDecoderMp3::createDecompiler()
	{
	    //查找解码器
        ...
        if( ret < 0) {
            printf("AudioFileDecoderMp3:: find avcodec faile!!!\n");
            avformat_close_input(&d->pFormatCtx);
            return;
        }
	    ...
	    if (d->pCodec == NULL){
		    printf("AudioFileDecoderMp3:: Codec not found\n");
		    avformat_close_input(&d->pFormatCtx);
		    return;
	    }
	    ...
	    if (avcodec_open2(d->pCodecCtx, d->pCodec, NULL) < 0) {
		    printf("AudioFileDecoderMp3:: Could not open codec\n");
    ...
    void AudioFileDecoderMp3::openAudioDevice()
	{
        qDebug() << "AudioFileDecoderMp3::readData() channels=" << d->pCodecCtx->channels
        ...
        qDebug() << "AudioFileDecoderMp3::OpenAudio() d->wanted_spec.channels== " <<
        ...
        qDebug() << "AudioFileDecoderMp3::OpenAudio() d->spec.channels== " <<
    ...
    void AudioFileDecoderMp3::readData()
    {
        //分配一个packet
        ...
        while(!d->isAudioPacketReadFinished)
        {
            ...
            if (packet->stream_index == d->audioIndex)
            {
                ...
                if(!d->isStartToPlay) {
                    ...
                    qDebug() << "AudioFileDecoderMp3::readData() satrt play audio@@@@@@@@";
                }
            } else {
                av_free_packet(packet);
            }
        }
        qDebug() << "AudioFileDecoderMp3::readData packet finished!!!size="
...
//三个变量设置为static就是为了保存上次数据，也可以用全局变量，但是感觉这样更好。
    void AudioFileDecoderMp3::audio_callback(void *userdata, Uint8 *stream, int len)
    {
    ...
    int AudioFileDecoderMp3::audio_decode_frame(AVCodecContext *aCodecCtx)
    {
        static AVPacket pkt;
        ...
        if( packet_queue_empty(&d->audioq) && d->isAudioPacketReadFinished ) {
            qDebug() << "AudioFileDecoderMp3::audio_decode_frame finished!!!!";
            SDL_PauseAudio(1);
    ...
    void AudioFileDecoderMp3::setAudioDecoderState(bool flag)
    {
        d->isRuning = flag;
        qDebug() << "AudioFileDecoderMp3::setAudioDecoderState flag=" << flag;
    }

    bool AudioFileDecoderMp3::getAudioDecoderState()
    {
        return d->isRuning;
    }

    void AudioFileDecoderMp3::freeMemory()
    {
    ...
    int AudioFileDecoderMp3::getAudioPacketSize()
    {
        return packet_queue_Size(&d->audioq);
    }

    void AudioFileDecoderMp3::play()
    {
        d->isRuning = true;
        startParse();
    }

	void AudioFileDecoderMp3::stop()
	{
	    qDebug() << "AudioFileDecoderMp3::stop()!!!!!!!!!";
	    isMusicPlayFinished_GL = true;
        d->isRuning = false;
        quitPlay();
        freeMemory();
	...
	void AudioFileDecoderMp3::suspendPlay()
	{
	    d->isRuning = false;
	    SDL_PauseAudio(1);
	}

	void AudioFileDecoderMp3::keepOnPlay()
	{
	    d->isRuning = true;
	    SDL_PauseAudio(0);
	}

	const char *AudioFileDecoderMp3::getAudioPlayerType()
	{
	    return AUDIO_MSG[FILE_MP3].c_str();
	}

	void AudioFileDecoderMp3::setFilePath(const std::string &path)
	{
	    d->filePath = path;
	}

	bool AudioFileDecoderMp3::getPlayFinishedState()
	{
	    return d->isPLayFinished;
	}

	void AudioFileDecoderMp3::quitPlay()
	{
	...
	void AudioFileDecoderMp3::adjustPlayTimePos(int timePos)
	{
	    d->isRuning = false;
	    if(SDL_GetAudioStatus() == SDL_AUDIO_PLAYING) {
	        SDL_PauseAudio(1);
	...
	void AudioFileDecoderMp3::avSeekPacket(int timestamp)
    {
        ...
        if(ret < 0) {
            qDebug() << "AudioFileDecoderMp3::avSeekPacket failed!!!!!!!!!!";
            d->isRuning = true;
            SDL_PauseAudio(0);
            return;
        }
        ...
        while(!d->isAudioPacketReadFinished)
        {
            ...
            if (packet->stream_index == d->audioIndex)
            {
                ...
                //如果没有这个就放不出声音,0是不暂停，非零是暂停
                    ...
                    qDebug() << "AudioFileDecoderMp3::avSeekPacket satrt play audio!!";
                }
            } else {
                av_free_packet(packet);
            }
        }
        qDebug() << "AudioFileDecoderMp3::avSeekPacket finished ,,size="
                    << packet_queue_Size(&d->audioq);
	}

    void AudioFileDecoderMp3::createAudioFile()
    {
